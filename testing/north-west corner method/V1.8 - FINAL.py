...
class Application():
    ...
    def MainWindow(self): # the windows will be in seperate functions so that they can be created again.
        ...
        Bhelp = tk.Button(self.root, text = "help", command = lambda: self.helps(1))
...
for example if there are 4 suppliers and 3 customers, then row is 4 and column is 3.
...
the values have to be integers and be between 3 to 10.""")
            ...
            tk.messagebox.showinfo("help","""the text boxes in line with the 'S1','S2','D1','D2' and so forth are the delivery costs. the text boxes below and beside the 'SUPPLY' and 'DEMAND' respectifully are the total stock amount have/need
the tick box next to 'include iterations?' is an option to include solutions during finding the optimal solution into the excel file. this is a useful tool for students.""")
...
please try again. if you need more help, please click the help button.""")
            ...
            tk.messagebox.showwarning("invalid values",""" the values you entered are not numbers, negative numbers or no value has been entered at all.please try again. if you need more help, please click the help button.""")
...
version: 1.1 """)
    ...
    def CgridWindow(self): # this creates the "create" grid window. this allows the user to enter the delivery cost of each number
        ...
        frames = []
        for a in range(int(self.rows.get())*2 + 3): # the frames were created so that the correct widgets can be used.
        ...
        nameCounter = 0
        #print(namesRow)
        for a in range(2,int(self.rows.get())*2+3 - 1,2): # this uses the frames between the horizontal lines and starts the placements of widgets.
            frames[a*(int(self.columns.get())*2+3)] = tk.Label(frames[a*(int(self.columns.get())*2+3)],text = namesRow[nameCounter])### # this just places the name of the row.
            nameCounter += 1
            #print(nameCounter)
            for b in range(1,int(self.columns.get())*2+3 - 1,2): # this creates the widgets that places the entry boxes and the vertical lines.
                frames[a*(int(self.columns.get())*2+3)+b] = tk.Label(frames[a*(int(self.columns.get())*2+3)+b],text = "|")###
                frames[a*(int(self.columns.get())*2+3)+b+1] = tk.Entry(frames[a*(int(self.columns.get())*2+3)+b+1],width = 10)###
        ...
        last = (int(self.columns.get())*2+3)*((int(self.rows.get())*2+3) - 1)# is the last line of the grid that is controlled differently than the others, just like the first line of the grid.
        frames[last] = tk.Label(frames[last], text = namesRow[-1]) # the last line is like this: NLELELELE...B (N = text,L = line,E = entry box,B = blank), so there is custom code that creates this seperatly.
        for b in range(1,int(self.columns.get())*2 + 1,2):
                frames[last+b] = tk.Label(frames[last+b],text = "|")
                frames[last+b+1] = tk.Entry(frames[last+b+1], width = 10)
        ...
        Bsolve = tk.Button(self.Cgrid,text = "solve", command = lambda: self.transfer(EntryPlace,frames,area))# this creates the buttons and check box needed.
        Bhelp = tk.Button(self.Cgrid,text = "help", command = lambda: self.helps(2))# this opens help function and the desired message.
        Breturn = tk.Button(self.Cgrid,text = "return", command = lambda: self.quits(2)) # this returns the user back to the main window.
        Citerations = tk.Checkbutton(self.Cgrid, text = "include iterations?",onvalue = 1, offvalue = 0, height = 5, width = 20, variable = self.iterations)# this will decide if the user wants to show iterations of the solution or not in the excel file

        Bsolve.grid(row = (int(self.rows.get())*2 + 4), column = 8)
        Bhelp.grid(row = (int(self.rows.get())*2 + 4), column = 6)
        Breturn.grid(row = (int(self.rows.get())*2 + 4), column = 4)
        Citerations.grid(row = (int(self.rows.get())*2 + 4), column = 1, columnspan = 3)
        
    
        self.Cgrid.mainloop()
    
    def transfer(self,EntryPlace,frames,area): # this transfer the data recieved in the entry boxes into array that has organised information.
        EntryValues = []
        deliveryCosts = []
        supply = []
        demand = []
        ...
        if checked == True: # if the values have passed the test, then the function is continued.
            ...
            for a in range(area[0]):
                ...
                for b in range(area[1]):
                    rowStore.append(EntryValues[a*(area[1]+1)+b]) # this "follows" the shape of the grid, so that the right information is sorted.
                    #print(rowStore)
                supply.append(EntryValues[a*(area[1]+1) + area[1]])
                deliveryCosts.append(rowStore)
                rowStore = []
    ...
    def checkGValues(self,EntryValues): # this checks if the string entered are positive and only contain numbers
        values = EntryValues
        try:
