...
version: 0.5 """)
    ...
    def check(self,which):
        try:
            ...
            if (x >= 3 and x <= 10): # this checks if the numbers are in range, if not a error is rasied.
                if y >= 3 and y<= 10:
                    if which == 1:
                        return self.Cgrid() # this will call the create grid function, leading the user to create the grid.
                    if which == 2:
                        return self.Rgrid() # this will call the random grid function, leading the user to view a randomly-ish generated grid.
    ...
    def Cgrid(self):
        print("YES this is create. rows is " + self.rows.get() + " and columns are " + self.columns.get())
