...
version: 1.1 """)
    ...
    def CgridWindow(self):
        ...
        Bhelp = tk.Button(self.Cgrid,text = "help", command = lambda: self.helps(2))
        Breturn = tk.Button(self.Cgrid,text = "return", command = lambda: self.quits(2))
        Citerations = tk.Checkbutton(self.Cgrid, text = "include iterations?",onvalue = 1, offvalue = 0, height = 5, width = 20, variable = self.iterations)
        ...
        if checked == True:
            ...
            for a in range(area[0]):
                ...
                for b in range(area[1]):
                    rowStore.append(EntryValues[a*(area[1]+1)+b])
                    #print(rowStore)
