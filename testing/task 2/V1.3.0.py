...
version: 1.1 """)
    ...
    def CgridWindow(self):
        ...
        for i in range(1,int(self.columns.get())*2 + 3,2):
            frames[i] = tk.Label(frames[i],text = "L")

        for i in range(2,int(self.columns.get())*2 + 3,2):
            frames[i] = tk.Label(frames[i],text = "N")

        for a in range(1,int(self.rows.get())*2 + 3,2):
            for b in range(int(self.columns.get())*2+3):
                frames[a*11+b] = tk.Label(frames[a*11+b],text = "L")

        for a in range(2,int(self.rows.get())*2+3 - 1,2):
            frames[a*11] = tk.Label(frames[a*11],text = "N")
            for b in range(1,int(self.columns.get())*2+3 - 1,2):
                frames[a*11+b] = tk.Label(frames[a*11+b],text = "L")
                frames[a*11+b+1] = tk.Label(frames[a*11+b+1],text = "E")
        ...
        last = 11*((int(self.rows.get())*2+3) - 1)
        frames[last] = tk.Label(frames[last], text = "N")
        for b in range(1,int(self.columns.get())*2 + 1,2):
                frames[last+b] = tk.Label(frames[last+b],text = "L")
                frames[last+b+1] = tk.Label(frames[last+b+1],text = "E")
                #carry = b
        frames[last+int(self.columns.get())*2 + 1] = tk.Label(frames[last+int(self.columns.get())*2 + 1], text = "L")
        frames[last+int(self.columns.get())*2 + 2] = tk.Label(frames[last+int(self.columns.get())*2 + 2],text = "B")
